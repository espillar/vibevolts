...
A detailed description of the `sim_data` dictionary can be found in `datastructure.md`.
...
*   `propagate_satellites(data_struct: Dict[str, Any], time_date: datetime, sat_category: str = None) -> Dict[str, Any]`: Updates satellite positions and pointing vectors based on their orbital elements.
...
### `pointing.py`
*   `generate_pointing_sphere(sim_data: Dict[str, Any], n_points: int, debug: bool = False) -> None`: Generates a pointing sphere with n_points and stores it in the sim_data['pointing_sphers'][n_points].
...
*   `setDetectorIntegrationTime(sim_data, itime)`: setDetectorFOV goes through the detectors in sim_data and changes the FOVs of all of them to size (radians).
...
### `demo1.py`
*   `demo1() -> go.Figure`: Runs a full demonstration of the simulation tools.

### `demo2.py`
*   `demo2() -> go.Figure`: Runs a demonstration plotting satellite and celestial positions.

### `demo3.py`
*   `demo3() -> go.Figure`: Runs a demonstration plotting a single LEO satellite trajectory.

### `demogeo.py`
*   `demo4() -> go.Figure`: Runs a demonstration plotting a single GEO satellite trajectory.

### `demo_common.py`
*   `initialize_standard_simulation(start_time=None) -> Dict[str, Any]`: Initializes a standard simulation with a predefined set of satellites.
...
*   `demo_exclusion_table() -> go.Figure`: Demonstrates the creation and visualization of the exclusion table.
...
*   `demo_pointing_plot() -> go.Figure`: Demonstrates the plot_pointing_vectors function.
...
*   `demo_pointing_vectors() -> go.Figure`: Demonstrates the generation and plotting of pointing vectors.
