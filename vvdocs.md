...
*   **`detector.py`**: 
    *   `makeBlankDetector(n)`: 
    *   `makeDetector(n, band, fov, ifov, aper, qe=0.5, photfrac=0.7, solarex=20 * DEGREE, lunarex=10 * DEGREE, earthex=15 * DEGREE)`: makeDetector takes parameters of a sensor and stuffs a filter array and a detector array, which it returns.
...
THIS VERSION IS FOR A GROUND OBSERVAOTRY
    *   `requiredIntegrationTime(limitingMag, SNR, filt, d, debug=0)`: requiredIntegrationTime(limitingMag, d)
...
to the conventional names used in that paper.
    *   `testdetector()`: testdetector creates an example that can be compared
...
Returns:
    The Plotly figure object.
*   **`demo_lambertian.py`**: 
    *   `demo_lambertian()`: Runs a demonstration of the lambertiansphere function,
...
Returns:
    ...
    or photons / s / m^2).
    *   `lambertiansphere(vec_from_sphere_to_light: np.ndarray, vec_from_sphere_to_observer: np.ndarray, albedo: float, radius: float)`: Calculates the effective brightness of a
Lambertian sphere.
...
*   **`plot_satellite_brighness.py`**: 
    *   `plot_satellite_brightness()`: Plots the apparent V-band photon flux and magnitude of satellites
...
0 if its     POINTING_COUNT_IDX or greater.
    *   `jerk(data_struct: Dict[str, Any], satellite_number: int)`: Moves the pointing vector of a specific satellite by 0.3 radians in a
random direction.
...
Returns:
    ...
    *   `blackbody_flux(temperature: float, lambda_short: float, lambda_long: float)`: Numerically computes the integrated spectral radiance of
...
Returns:
    ...
    Watts / (m^2 * steradian).
    *   `stefan_boltzmann_law(temperature: float)`: Calculates the total power radiated per unit area by a
...
Returns:
    ...
    *   `plot_blackbody_spectrum(temperature: float)`: Plots the spectral radiance of a blackbody from
0.5 to 30 microns.

Args:
    temperature: The temperature of the blackbody in Kelvin.
    *   `plot_blackbody_spectrum_visible_nir(temperature: float)`: Plots the spectral radiance of a blackbody from
0.1 to 1 micron.

Args:
    temperature: The temperature of the blackbody in Kelvin.
    *   `sat_magnitude(size: float, range: float, angle: float, band: str)`: given a satellite size and a waveband and range
pull the brightness of the sun and the calibration from radiometry_data
*   **`show_geo_search.py`**: 
    *   `show_geo_search()`: This demo initializes a simulation, adds a GEO constellation, and then
...
*   **`tests/test_simulation.py`**: 
    *   `test_create_empty_simulation_structure()`: Tests that create_empty_simulation returns a dictionary
