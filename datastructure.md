...
| | | `counts['satellites']` (int): Number of satellites. | `constellation.geos`, `constellation.geosmod`, `propagation.add_satellites_from_tle` |
| | | `counts['observatories']` (int): Number of observatories. | `observatories.add_observatories` |
| | | `counts['fixedpoints']` (int): Number of fixed points. | `targets.add_fixed_points` |
...
| `celestial` | `dict` | Dictionary containing data for celestial bodies (Sun, Moon). | `celestialbodies.add_celestial_bodies` |
...
| `satellites`| `dict` | Dictionary containing data for satellites. | `constellation.geos`, `constellation.geosmod`, `propagation.add_satellites_from_tle` |
...
| `fixedpoints`| `dict` | Dictionary for fixed reference points in the GCRS frame. | `targets.add_fixed_points` |
| | | `fixedpoints['position']` (np.ndarray): `(n, 3)` array of GCRS positions. | |
| | | `fixedpoints['exclusion']` (np.ndarray): `(n,)` array of exclusion flags. | |
...
| `detector` | `SimpleNamespace` | A namespace object containing detector parameters for all satellites/observatories. | `constellation.geos`, `constellation.geosmod`, `observatories.add_observatories`, `propagation.add_satellites_from_tle`, `detector.makeBlankDetector`, `detector.makeDetector` |
...
| Key | Type | Description | Functions Modifying This Key |
| --- | --- | --- | --- |
| `aperture` | `np.ndarray(float)` | Aperture area in square meters. | `detector.makeDetector` |
| `pixelArea` | `np.ndarray(float)` | Pixel area in square arcseconds. | `detector.makeDetector` |
...
| `solarEx` | `np.ndarray(float)` | Solar exclusion angle in radians. | `detector.makeDetector`, `demo_sky_scan`, `demo_exclusion_table` |
| `lunarex` | `np.ndarray(float)` | Lunar exclusion angle in radians. | `detector.makeDetector`, `demo_sky_scan`, `demo_exclusion_table` |
| `earthEx` | `np.ndarray(float)` | Earth exclusion angle (above the limb) in radians. | `detector.makeDetector`, `demo_sky_scan`, `demo_exclusion_table` |
...
| `itime` | `np.ndarray(float)` | Integration time required to reach a desired limiting magnitude. | `detector.makeDetector` |
| `fov` | `np.ndarray(float)` | Field of view in radians. | `detector.makeDetector` |
