...
\title{VibeVolts Project Documentation}
\author{Jules the Pirate Engineer}
\date{\today}
...
VibeVolts is a Python-based simulation toolkit for space environment modeling. It provides a set of tools to initialize, propagate, and analyze the state of various space-based and ground-based assets. The project's primary goal is to create a fast and efficient simulation tool for modeling satellite and observatory visibility of objects in space, with a focus on performance by leveraging vectorized operations with NumPy.

The toolkit is structured into several core modules, each responsible for a specific part of the simulation, such as orbit propagation (`propagation.py`), visibility calculations (`visibility.py`), and satellite pointing (`pointing.py`). The entire simulation state is managed within a single dictionary, `simulation_data`, which is initialized by composition. A minimal simulation is created with `create_empty_simulation` and then populated by functions like `add_satellites_from_tle`, `add_observatories`, etc.

VibeVolts includes a number of demonstration scripts that showcase its features. These demos cover everything from basic satellite propagation to more complex scenarios like sky scans and exclusion zone calculations. The `all\_demos.py` script provides a convenient way to run all the demonstrations.

\section{Python Code Listings}

\subsection{all\_demos.py}
\begin{lstlisting}
...
\subsection{demo\_exclusion\_debug\_print.py}
\begin{lstlisting}
import numpy as np
...
\subsection{demo\_exclusion\_table.py}
\begin{lstlisting}
import numpy as np
from datetime import datetime, timezone
import plotly.graph_objects as go
...
\subsection{demo\_fixedpoints.py}
\begin{lstlisting}
from datetime import datetime, timezone
import plotly.graph_objects as go
...
\subsection{demo\_lambertian.py}
\begin{lstlisting}
import numpy as np
import plotly.graph_objects as go
...
\subsection{demo\_pointing\_plot.py}
\begin{lstlisting}
from datetime import datetime, timezone
import plotly.graph_objects as go
...
\subsection{demo\_pointing\_sequence.py}
\begin{lstlisting}
import numpy as np
from datetime import datetime, timezone, timedelta
import plotly.graph_objects as go
...
\subsection{demo\_pointing\_vectors.py}
\begin{lstlisting}
import plotly.graph_objects as go
...
\subsection{generate\_log\_spherical\_points.py}
\begin{lstlisting}
import numpy as np
from datetime import datetime, timezone
from plotting_3d import plot_3d_scatter
...
\subsection{generate\_report.py}
\begin{lstlisting}
import plotly.graph_objects as go
...
\subsection{plotting\_vectors.py}
\begin{lstlisting}
import numpy as np
from datetime import datetime
from typing import Dict, Any
...
\subsection{pointing\_vectors.py}
\begin{lstlisting}
import numpy as np
import plotly.graph_objects as go
...
\subsection{radiometry\_calcs.py}
\begin{lstlisting}
import math
...
\subsection{radiometry\_data.py}
\begin{lstlisting}
import scipy.constants as const
