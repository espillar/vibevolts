...
The toolkit is structured into several core modules, each responsible for a specific part of the simulation, such as orbit propagation (`propagation.py`), visibility calculations (`visibility.py`), and satellite pointing (`pointing.py`). The entire simulation state is managed within a single dictionary, `simulation\_data`, which is initialized by the `simulation.py` module.
