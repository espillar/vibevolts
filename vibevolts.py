...
def initializeStructures(
    ...
    The 'fixedpoints' component is initialized with a set of 100 static points
    in the GCRS frame, generated by `generate_log_spherical_points`. These
    points have a logarithmic radial distribution and come with an associated
    size. The structure is as follows:
    - 'position': A NumPy array (N, 3) of Cartesian coordinates.
    - 'size': A NumPy array (N,) of object sizes in meters.
    ...
    simulation_data: Dict[str, Any] = {
        ...
        'fixedpoints': {
            'position': np.zeros((100, 3), dtype=float),
            'size': np.zeros(100, dtype=float)
        }
    }

    # Generate and assign fixed points and their sizes
    fixed_points_pos, fixed_points_size = generate_log_spherical_points(
        num_points=100,
        inner_radius=2000000,
        outer_radius=84328000,
        object_size_m=1.0  # Default size of 1 meter
    )
    simulation_data['fixedpoints']['position'] = fixed_points_pos
    simulation_data['fixedpoints']['size'] = fixed_points_size
