...
- *~pixelOmega~*: ~np.ndarray~ (n,) - Pixel area in square arcsec.
...
- *~integrationTime~*: ~np.ndarray~ (n,) - Integration time.
- *~fov~*: ~np.ndarray~ (n,) - Field of view.
- *~ifov~*: ~np.ndarray~ (n,) - Instantaneous field of view.
- *~filt~*: ~list[str]~ (n) - Filter name.
- *~category~*: ~list[str]~ (n) - Category of the asset (e.g. 'satellites').
- *~asset_index~*: ~np.ndarray~ (n,) - Index of the asset within its category.
- *~pointing~*: ~np.ndarray~ (n, 3) - Pointing direction vector.
