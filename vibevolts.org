...
** Details
*** Coding [12/26]
**** Get a simple search pattern going [2/11]
- [X] update sensor fields of view-make sure they are OK
- [X] Get detectors declared in initialize_standard_simulation
- [ ] iterate through the sensors, and for each create the dot product between their FOV and all the unit vectors to the satellites.
  This in detector.py
...
- [ ] check that lambertian sphere works adequantely for vectors of
  all the pieces
...
**** Tasks for AI [/]
- [ ] Respin documentation. Specifically,I think there should be a section
  for each of the items in sim_data descibing whats in there, and an
  additional section fro constants and for radiomentry_data.
