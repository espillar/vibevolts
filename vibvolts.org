...
OK, had some thoughts on this:
We can probably assign satellites to do some simple patterns
that follow their strengths in terms of where they should be
looking, we reallly only have a few categories I think
we are going to try.
...
Tried to have jules fix the docs up and get some demos to run- but it tried to
insert a matplolib and a pandas dependency that I have asked it to remove.


**** prompt

Add a new two dimensional numpy array to the common area.
The first index should correspond to a satellite number.
The second number should correspond to a fixed point.

update both vvdoc files to keep up with the current functions and structures.
