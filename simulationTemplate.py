...
def run_simulation_template():
    ...
    Shows the basic lifecycle: Initialize -> Add Components -> Loop (Update & Propagate)
    """
    # 1. Initialize
    ...
    print(f"Radius range: {np.min(fixed_radii)/GEO_RADIUS:.2f} to {np.max(fixed_radii)/GEO_RADIUS:.2f} GEO_RADIUS")
    ...
    print(f"Simulation initialized with {sim_data['counts']['satellites']} satellites.")

    # 3. Simulation Loop
    for step in range(20):  # Run for 20 steps

        # cadenceController.nextIntegration advances time, propagates satellites, and scans
        # Now it also updates celestial body positions.
