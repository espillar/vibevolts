...
- *create_empty_simulation(...)*: Creates a minimal, empty data structure for a space simulation.
- *add_celestial_bodies(...)*: Adds celestial body structures (for Sun and Moon) to the simulation data.
- *add_fixed_points(...)*: Adds a structure for fixed reference points in the GCRS frame.
...
- *jerk(...)*: Moves the pointing vector of a specific satellite by 0.3
  radians in a random direction.
- *find_and_jerk_blind_satellites(...)*: Finds satellites with no
